...
export default function DeleteModal(props) {
    ...
    const {openModal, openModalHandler, form, session, userId} = props;
    ...
    const mutation = useMutation({
        mutationFn: async (data) => {
            return await deleteUser(data.header, data.userId)
        },
        onSuccess: () => {
        ...
        <div className={'flex flex-col'}>
            <div className={'mx-5'}>
                <h1 className={'my-5 text-slate-500'}>Delete <span
                    className={'text-slate-700'}>{form.firstName} {form.lastName}</span>?</h1>
                {confirmError &&
                    <p className={'mb-2 px-2 bg-red-400 rounded-lg text-white'}>Please type the exact word case
                        sensitive</p>}
                <Input
                    label={'Type CONFIRM to delete user'}
                    inputBold={true}
                    withButton={true}
                    value={confirm}

...
import {deleteShift} from "../../helpers/api-utils/shifts";

export default function DeleteTeamMember(props) {

    const {openModal, openModalHandler, form, session, shiftMemberId} = props;
    ...
    const mutation = useMutation({
        mutationFn: async (data) => {
            return await deleteShift(data.header, data.id)
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['shiftMembers']);
            queryClient.refetchQueries('shiftMembers', {force: true});
        }
    });
        ...
        <div className={'flex flex-col'}>
            <div className={'mx-5'}>
                <h1 className={'my-5 text-slate-500'}>Delete <span
                    className={'text-slate-700'}>{form?.user?.fullName}</span>?</h1>
                {confirmError &&
